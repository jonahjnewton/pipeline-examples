def export_usd_animcache(export_node, filepath, start_frame, end_frame, frame_stride, rig='', publish_path='', skelRoot = '', frameHold=0):
    ...
    try:
        ...
        with utils.maya_keep_parent(reparent_node) as rp_node:
            ...
            # If skelRoot is set, export full rig, not just geo cache.
            if skelRoot:
                ...
                if(cmds.referenceQuery(reparented_export_node, inr=True)):
                    ...
                    for mesh in mesh_xforms:
                        try:
                            ...
                            rest_points = mesh.getPoints()
                            rest_points_array = [(x.x, x.y, x.z) for x in rest_points]
                            
                            sdf_path = '/'.join([x.split(':')[-1] for x in dag_path.split('|')])

                            mesh_prim = stage.GetPrimAtPath(sdf_path)

                            primvarsapi = UsdGeom.PrimvarsAPI(mesh_prim)

                            primvar = primvarsapi.CreatePrimvar('Pref', Sdf.ValueTypeNames.Float3Array, UsdGeom.Tokens.vertex)
                            primvar.Set(rest_points_array)
                        except:
                            ...
                            for child_mesh in pm.listRelatives(mesh, children=True):
                                try:
                                    if "Orig" not in child_mesh.name() and "Deformed" not in child_mesh.name():
                                        rest_points = child_mesh.getPoints()
                                        dag_path = mesh.longName()
                                        ...
                                        primvar = primvarsapi.CreatePrimvar('Pref', Sdf.ValueTypeNames.Float3Array, UsdGeom.Tokens.vertex)
                                        primvar.Set(rest_points_array)
                                        child_mesh_success = True
                                        break
                                except:
                                    pass
                            
                            if not child_mesh_success:
                                print("ERROR Setting Pref on",mesh.longName())
                        

                    modelref_updates = {}
                    for modelref in modelrefs:
                        #Add USD layer references for other departments to internal model references
                        ...
                        if len(templates) > 0:
                            ...
                            #Move each child of the model under the new geo prim.
                    ...
                    if frameHold > 1:
                        ...
                        # For each attribute with time samples, hold the value
                        attrTimeSampleDict = {}
                        ...
                        # Create a variant for each possible frame hold start frame 
                        # (e.g. if on 2's, need a variant for held odd frames and a variant for held even frames)
                        ...
                        # Create a variant for normal frames (on 1's)
